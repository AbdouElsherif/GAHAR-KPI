...
import {
    TechnicalClinicalFacility,
    saveTechnicalClinicalFacility,
    getTechnicalClinicalFacilities,
    updateTechnicalClinicalFacility,
    deleteTechnicalClinicalFacility
} from '@/lib/firestore';

interface TechnicalClinicalFacilitiesSectionProps {
    currentUser: any;
...
/**
 * مكون قسم "المنشآت التي تم زيارتها خلال الشهر" لـ dept4
 */
export default function TechnicalClinicalFacilitiesSection({ currentUser, canEdit }: TechnicalClinicalFacilitiesSectionProps) {
    // State
    const [facilities, setFacilities] = useState<TechnicalClinicalFacility[]>([]);
    ...
    const loadData = async () => {
        const data = await getTechnicalClinicalFacilities();
        setFacilities(data);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                const success = await updateTechnicalClinicalFacility(editingId, {
                    ...facilityData,
                    updatedBy: currentUser.email
                });
                ...
                const id = await saveTechnicalClinicalFacility(facilityData);
                if (id) {
                    await loadData();
                    resetForm();
    ...
    const handleEdit = (item: TechnicalClinicalFacility) => {
        setFormData({
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(workbook, `المنشآت_الفنية_الإكلينيكية${filterMonthText}.xlsx`);
    };
    ...
    const exportToWord = async () => {
        ...
        const doc = new Document({
            sections: [{
                ...
                children: [
                    new Paragraph({
                        text: 'المنشآت التي تم زيارتها خلال الشهر - الرقابة الفنية والإكلينيكية',
        ...
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `المنشآت_الفنية_الإكلينيكية${filterMonthText}.docx`;
        link.click();
    };
