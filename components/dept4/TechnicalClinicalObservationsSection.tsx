...
import {
    TechnicalClinicalObservation,
    saveTechnicalClinicalObservation,
    getTechnicalClinicalObservations,
    updateTechnicalClinicalObservation,
    deleteTechnicalClinicalObservation
} from '@/lib/firestore';

interface TechnicalClinicalObservationsSectionProps {
    currentUser: any;
    canEdit: (user: any) => boolean;
}

/**
 * مكون قسم "الملاحظات المتكررة" لـ dept4
 */
export default function TechnicalClinicalObservationsSection({ currentUser, canEdit }: TechnicalClinicalObservationsSectionProps) {
    // State
    const [observations, setObservations] = useState<TechnicalClinicalObservation[]>([]);
    ...
    const loadData = async () => {
        const data = await getTechnicalClinicalObservations();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                const success = await updateTechnicalClinicalObservation(editingId, {
                    ...observationData,
                    updatedBy: currentUser.email
                });
                ...
                const id = await saveTechnicalClinicalObservation(observationData);
                if (id) {
                    await loadData();
                    resetForm();
    ...
    const handleEdit = (item: TechnicalClinicalObservation) => {
        setFormData({
    ...
    const handleDelete = async (id: string) => {
        ...
        if (confirm('هل أنت متأكد من حذف هذا السجل؟')) {
            const success = await deleteTechnicalClinicalObservation(id);
            if (success) {
                await loadData();
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(workbook, `الملاحظات_المتكررة_الفنية${filterMonthText}.xlsx`);
    };
    ...
    const exportToWord = async () => {
        ...
        const doc = new Document({
            sections: [{
                ...
                children: [
                    new Paragraph({
                        text: 'الملاحظات المتكررة - الرقابة الفنية والإكلينيكية',
        ...
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `الملاحظات_المتكررة_الفنية${filterMonthText}.docx`;
        link.click();
    };
                        ...
                        <form onSubmit={handleSubmit} style={{ marginBottom: '30px', padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
                                ...
                                <div className="form-group" style={{ gridColumn: 'span 2' }}>
                                    <label className="form-label">الملاحظة / دليل التطابق *</label>
                                    <textarea
                                        className="form-input"
                                        required
                                        value={formData.observation}
                                        onChange={(e) => setFormData({ ...formData, observation: e.target.value })}
                                        placeholder="اكتب الملاحظة أو دليل التطابق"
                                        rows={3}
