...
export default function QueuedSupportVisitsSection({ currentUser, canEdit }: QueuedSupportVisitsSectionProps) {
    const [visits, setVisits] = useState<QueuedSupportVisit[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                if (await updateQueuedSupportVisit(editingId, data)) {
                    await loadData();
                    resetForm();
                    alert('تم التحديث بنجاح');
                }
            } else {
                if (await saveQueuedSupportVisit(data)) {
                    await loadData();
                    resetForm();
    ...
    const resetForm = () => {
        setFormData({ facilityName: '', governorate: '', month: '' });
        setEditingId(null);
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        if (confirm('هل أنت متأكد؟')) {
            if (await deleteQueuedSupportVisit(id)) {
                await loadData();
                alert('تم الحذف');
            }
        }
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(data), 'زيارات قائمة الانتظار');
        XLSX.writeFile(wb, 'زيارات_قائمة_الانتظار.xlsx');
    };
    ...
    const exportToWord = async () => {
        ...
        const doc = new Document({ sections: [{ children: [new Paragraph({ text: 'زيارات الدعم الفني بقائمة الانتظار', heading: 'Heading1', alignment: AlignmentType.CENTER }), table] }] });
        const blob = await Packer.toBlob(doc);
        const link = document.createElement('a');
        link.href = window.URL.createObjectURL(blob);
        link.download = 'زيارات_قائمة_الانتظار.docx';
        link.click();
    };

    return (
        <div className="card" style={{ marginTop: '20px' }}>
            <SectionHeader title="زيارات الدعم الفني بقائمة الانتظار" icon="⏳" count={filteredData.length} isExpanded={isExpanded} onToggle={() => setIsExpanded(!isExpanded)} />
            {isExpanded && (
                <>
                    {userCanEdit && (
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table className="data-table">
                            ...
                            <tbody>
                                {filteredData.length === 0 ? <tr><td colSpan={5} style={{ textAlign: 'center', padding: '20px' }}>لا توجد بيانات</td></tr> : filteredData.map((d, i) => (
                                    <tr key={d.id}><td>{i + 1}</td><td>{d.facilityName}</td><td>{d.governorate}</td><td>{d.month}</td>
