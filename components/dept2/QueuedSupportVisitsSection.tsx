...
import {
    ...
    updateQueuedSupportVisit,
    deleteQueuedSupportVisit
} from '@/lib/firestore';

interface QueuedSupportVisitsSectionProps {
    currentUser: any;
...
/**
 * مكون قسم "زيارات الدعم الفني بقائمة الانتظار" لـ dept2
 */
export default function QueuedSupportVisitsSection({ currentUser, canEdit }: QueuedSupportVisitsSectionProps) {
    // State
    const [queuedVisits, setQueuedVisits] = useState<QueuedSupportVisit[]>([]);
    ...
    const loadData = async () => {
        const data = await getQueuedSupportVisits();
        setQueuedVisits(data);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.facilityName || !formData.governorate || !formData.month) {
        ...
        try {
            if (editingId) {
                const success = await updateQueuedSupportVisit(editingId, {
                    ...visitData,
                    updatedBy: currentUser.email
                });
                ...
                const id = await saveQueuedSupportVisit(visitData);
                if (id) {
                    await loadData();
                    resetForm();
    ...
    const handleEdit = (item: QueuedSupportVisit) => {
        setFormData({
    ...
    const handleDelete = async (id: string) => {
        ...
        if (confirm('هل أنت متأكد من حذف هذا السجل؟')) {
            const success = await deleteQueuedSupportVisit(id);
            if (success) {
                await loadData();
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'قائمة الانتظار');

        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        XLSX.writeFile(workbook, `زيارات_قائمة_الانتظار${filterMonthText}.xlsx`);
    };
    ...
    const exportToWord = async () => {
        ...
        const tableRows = filteredData.map((item, index) => {
            ...
            return new TableRow({
                children: [
                    ...
                    new TableCell({ children: [new Paragraph({ text: item.facilityName, alignment: AlignmentType.RIGHT })], width: { size: 50, type: WidthType.PERCENTAGE } }),
            ...
            rows: [
                new TableRow({
                    children: [
                        ...
                        new TableCell({ children: [new Paragraph({ text: 'اسم المنشأة', alignment: AlignmentType.CENTER })], width: { size: 50, type: WidthType.PERCENTAGE } }),
        ...
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `زيارات_قائمة_الانتظار${filterMonthText}.docx`;
        link.click();
    };

    // Filtering
    const filteredData = filterMonth
        ? queuedVisits.filter(v => v.month === filterMonth)
        : queuedVisits;
    ...
    return (
        <div className="card" style={{ marginTop: '30px' }}>
            <SectionHeader
                title="زيارات الدعم الفني بقائمة الانتظار"
                icon="⏳"
