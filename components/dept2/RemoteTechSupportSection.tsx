...
/**
 * مكون قسم "الدعم الفني عن بعد" لـ dept2
 */
export default function RemoteTechSupportSection({ currentUser, canEdit }: RemoteTechSupportSectionProps) {
    // State
    const [remoteSupports, setRemoteSupports] = useState<RemoteTechnicalSupport[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                const success = await updateRemoteTechnicalSupport(editingId, {
                    ...supportData,
                    updatedBy: currentUser.email
                });
                ...
                const id = await saveRemoteTechnicalSupport(supportData);
                if (id) {
                    await loadData();
                    resetForm();
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(workbook, `الدعم_الفني_عن_بعد${filterMonthText}.xlsx`);
    };
    ...
    const filteredData = filterMonth
        ? remoteSupports.filter(v => v.month === filterMonth)
        : remoteSupports;
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
                                ...
                                <div className="form-group">
                                    ...
                                    >
                                        ...
                                        <option value="متابعة عن بعد">متابعة عن بعد</option>
                                    </select>
                                </div>
                                <div className="form-group">
