...
export default function IntroductorySupportVisitsSection({ currentUser, canEdit }: IntroductorySupportVisitsSectionProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                const success = await updateIntroductorySupportVisit(editingId, {
                    ...visitData,
                    updatedBy: currentUser.email
                });
                ...
                const id = await saveIntroductorySupportVisit(visitData);
                if (id) {
                    await loadData();
                    resetForm();
    ...
    const handleDelete = async (id: string) => {
        ...
        if (confirm('هل أنت متأكد من حذف هذا السجل؟')) {
            const success = await deleteIntroductorySupportVisit(id);
            if (success) {
                await loadData();
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'الزيارات التمهيدية');

        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        XLSX.writeFile(workbook, `الزيارات_التمهيدية${filterMonthText}.xlsx`);
    };
    ...
    const exportToWord = async () => {
        ...
        const tableRows = filteredData.map((item, index) => {
            ...
            return new TableRow({
                children: [
                    ...
                    new TableCell({ children: [new Paragraph({ text: item.affiliatedEntity, alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } }),
        ...
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `الزيارات_التمهيدية${filterMonthText}.docx`;
        link.click();
    };

    // Filtering
    const filteredData = filterMonth
        ? introVisits.filter(v => v.month === filterMonth)
        : introVisits;
    ...
    return (
        <div className="card" style={{ marginTop: '30px' }}>
            <SectionHeader
                title="زيارات الدعم الفني التمهيدية"
                icon="🏥"
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
                                ...
                                <div className="form-group">
                                    ...
                                    >
                                        ...
                                        <option value="زيارة استكشافية">زيارة استكشافية</option>
                                ...
                                {filteredData.length === 0 ? (
                                    ...
                                    filteredData.map((item, index) => (
                                        <tr key={item.id} style={{ borderBottom: '1px solid #eee', backgroundColor: index % 2 === 0 ? 'white' : '#f9f9f9' }}>
                                            ...
                                            <td style={{ padding: '12px', textAlign: 'center' }}>{item.affiliatedEntity}</td>
