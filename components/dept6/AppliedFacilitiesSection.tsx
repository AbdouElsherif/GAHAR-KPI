...
// سنستخدم interface مبسط مؤقتاً
interface AppliedFacility {
    id?: string;
    facilityType: string;
    facilityName: string;
    governorate: string;
...
interface AppliedFacilitiesSectionProps {
    currentUser: any;
...
export default function AppliedFacilitiesSection({ currentUser, canEdit }: AppliedFacilitiesSectionProps) {
    const [facilities, setFacilities] = useState<AppliedFacility[]>([]);
    const [editingId, setEditingId] = useState<string | null>(null);
    const [filterMonth, setFilterMonth] = useState('');
    const [isExpanded, setIsExpanded] = useState(false);
    const [formData, setFormData] = useState({
        facilityType: '', facilityName: '', governorate: '', accreditationStatus: '', month: ''
    });
    ...
    const loadData = async () => {
        // TODO: استدعاء Firestore function
        // const data = await getAppliedFacilities();
        // setFacilities(data);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.facilityType || !formData.facilityName || !formData.governorate || !formData.accreditationStatus || !formData.month) {
        ...
        try {
            if (editingId) {
                // await updateAppliedFacility(editingId, { ...facilityData, updatedBy: currentUser.email });
                await loadData();
                resetForm();
                alert('تم التحديث');
            } else {
                // await saveAppliedFacility(facilityData);
                await loadData();
                resetForm();
                alert('تم الإضافة');
            }
    ...
    const resetForm = () => {
        setFormData({ facilityType: '', facilityName: '', governorate: '', accreditationStatus: '', month: '' });
        setEditingId(null);
    };

    const handleEdit = (item: AppliedFacility) => {
        setFormData({
    ...
    const handleDelete = async (id: string) => {
        ...
        if (confirm('هل أنت متأكد؟')) {
            // await deleteAppliedFacility(id);
            await loadData();
            alert('تم الحذف');
        }
    };
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'المنشآت المتقدمة');
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        XLSX.writeFile(workbook, `المنشآت_المتقدمة${filterMonthText}.xlsx`);
    };
    ...
    const exportToWord = async () => {
        ...
        const tableRows = filteredData.map((item, i) => {
            ...
            return new TableRow({
                children: [
                    new TableCell({ children: [new Paragraph({ text: (i + 1).toString(), alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }),
        ...
        const doc = new Document({ sections: [{ properties: {}, children: [new Paragraph({ text: 'المنشآت المتقدمة خلال الشهر', alignment: AlignmentType.CENTER, spacing: { after: 200 } }), table] }] });
        const blob = await Packer.toBlob(doc);
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `المنشآت_المتقدمة${filterMonthText}.docx`;
        link.click();
    };
    ...
    return (
        <div className="card" style={{ marginTop: '30px' }}>
            <SectionHeader title="المنشآت المتقدمة خلال الشهر" icon="📝" count={filteredData.length} isExpanded={isExpanded} onToggle={() => setIsExpanded(!isExpanded)} />
            {isExpanded && (
                <>
                    {userCanEdit && (
                        <form onSubmit={handleSubmit} style={{ marginBottom: '30px', padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="text" className="form-input" required value={formData.facilityName} onChange={(e) => setFormData({ ...formData, facilityName: e.target.value })} />
                                </div>
                                <div className="form-group">
