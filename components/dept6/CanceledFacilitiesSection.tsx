...
interface CanceledFacilitiesSectionProps {
    currentUser: any;
...
export default function CanceledFacilitiesSection({ currentUser, canEdit }: CanceledFacilitiesSectionProps) {
    const [facilities, setFacilities] = useState<CanceledFacilities[]>([]);
    ...
    const handleEdit = (item: CanceledFacilities) => { setFormData({ facilityType: item.facilityType, facilityName: item.facilityName, governorate: item.governorate, accreditationStatus: item.accreditationStatus, month: item.month }); setEditingId(item.id || null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
