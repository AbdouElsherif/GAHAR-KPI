...
interface RenewedFacilitiesSectionProps {
    currentUser: any;
...
export default function RenewedFacilitiesSection({ currentUser, canEdit }: RenewedFacilitiesSectionProps) {
    const [facilities, setFacilities] = useState<RenewedFacilities[]>([]);
    ...
    const handleEdit = (item: RenewedFacilities) => { setFormData({ facilityType: item.facilityType, facilityName: item.facilityName, governorate: item.governorate, accreditationStatus: item.accreditationStatus, month: item.month }); setEditingId(item.id || null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
