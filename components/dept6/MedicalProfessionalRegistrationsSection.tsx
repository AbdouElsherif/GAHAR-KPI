...
import { MedicalProfessionalRegistration } from '@/lib/firestore';

interface MedicalProfessionalRegistrationsSectionProps {
    currentUser: any;
...
export default function MedicalProfessionalRegistrationsSection({ currentUser, canEdit }: MedicalProfessionalRegistrationsSectionProps) {
    const [registrations, setRegistrations] = useState<MedicalProfessionalRegistration[]>([]);
    const [editingId, setEditingId] = useState<string | null>(null);
    const [filterMonth, setFilterMonth] = useState('');
    const [isExpanded, setIsExpanded] = useState(false);
    const [formData, setFormData] = useState({ facilityName: '', governorate: '', accreditationStatus: '', facilityType: '', month: '' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.facilityName || !formData.governorate || !formData.accreditationStatus || !formData.facilityType || !formData.month) { alert('يرجى ملء جميع الحقول'); return; }

        const [year, month] = formData.month.split('-');
        const regData = { ...formData, year: parseInt(year), createdBy: currentUser.email, updatedBy: currentUser.email };
    ...
    const resetForm = () => { setFormData({ facilityName: '', governorate: '', accreditationStatus: '', facilityType: '', month: '' }); setEditingId(null); };
    const handleEdit = (item: MedicalProfessionalRegistration) => { setFormData({ facilityName: item.facilityName, governorate: item.governorate, accreditationStatus: item.accreditationStatus, facilityType: item.facilityType, month: item.month }); setEditingId(item.id || null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
    ...
    const exportToExcel = () => {
        ...
        const data = filteredData.map((item, i) => { const [year, month] = item.month.split('-'); return { '#': i + 1, 'اسم المنشأة': item.facilityName, 'المحافظة': item.governorate, 'حالة الاعتماد': item.accreditationStatus, 'نوع المنشأة': item.facilityType, 'الشهر': `${monthNames[parseInt(month) - 1]} ${year}` }; });
        const worksheet = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, 'تسجيل المهنيين');
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        XLSX.writeFile(workbook, `تسجيل_المهنيين_الطبيين${filterMonthText}.xlsx`);
    };

    const exportToWord = async () => {
        const monthNames = ['يناير', 'فبراير', 'مارس', 'أبريل', 'مايو', 'يونيو', 'يوليو', 'أغسطس', 'سبتمبر', 'أكتوبر', 'نوفمبر', 'ديسمبر'];
        const tableRows = filteredData.map((item, i) => { const [year, month] = item.month.split('-'); return new TableRow({ children: [new TableCell({ children: [new Paragraph({ text: (i + 1).toString(), alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: item.facilityName, alignment: AlignmentType.RIGHT })], width: { size: 25, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: item.governorate, alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: item.accreditationStatus, alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: item.facilityType, alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: `${monthNames[parseInt(month) - 1]} ${year}`, alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } })] }); });
        const table = new Table({ rows: [new TableRow({ children: [new TableCell({ children: [new Paragraph({ text: '#', alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'اسم المنشأة', alignment: AlignmentType.CENTER })], width: { size: 25, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'المحافظة', alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'حالة الاعتماد', alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'نوع المنشأة', alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'الشهر', alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } })] }), ...tableRows], width: { size: 100, type: WidthType.PERCENTAGE } });
        const doc = new Document({ sections: [{ properties: {}, children: [new Paragraph({ text: 'تسجيل المهنيين الطبيين', alignment: AlignmentType.CENTER, spacing: { after: 200 } }), table] }] });
        const blob = await Packer.toBlob(doc);
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `تسجيل_المهنيين_الطبيين${filterMonthText}.docx`;
        link.click();
    };

    const filteredData = filterMonth ? registrations.filter(r => r.month === filterMonth) : registrations;
    ...
    return (
        <div className="card" style={{ marginTop: '30px' }}>
            <SectionHeader title="تسجيل المهنيين الطبيين" icon="👨‍⚕️" count={filteredData.length} isExpanded={isExpanded} onToggle={() => setIsExpanded(!isExpanded)} />
            {isExpanded && (
                <>
                    {userCanEdit && (
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
                                ...
                                <div className="form-group">
                                    ...
                                    <select className="form-input" required value={formData.facilityType} onChange={(e) => setFormData({ ...formData, facilityType: e.target.value })}>
                                        <option value="">اختر</option><option value="مستشفى">مستشفى</option><option value="مركز طبي">مركز طبي</option><option value="مركز رعاية أولية">مركز رعاية أولية</option><option value="عيادة">عيادة</option><option value="صيدلية">صيدلية</option><option value="معمل">معمل</option>
                                    </select>
                                </div>
