...
interface RegisteredFacilities {
    id?: string;
    facilityType: string;
    facilityName: string;
    governorate: string;
...
interface RegisteredFacilitiesSectionProps {
    currentUser: any;
...
export default function RegisteredFacilitiesSection({ currentUser, canEdit }: RegisteredFacilitiesSectionProps) {
    const [facilities, setFacilities] = useState<RegisteredFacilities[]>([]);
    ...
    const handleEdit = (item: RegisteredFacilities) => { setFormData({ facilityType: item.facilityType, facilityName: item.facilityName, governorate: item.governorate, accreditationStatus: item.accreditationStatus, month: item.month }); setEditingId(item.id || null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
