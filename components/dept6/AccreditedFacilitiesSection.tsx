...
interface AccreditedFacilitiesSectionProps {
    currentUser: any;
...
export default function AccreditedFacilitiesSection({ currentUser, canEdit }: AccreditedFacilitiesSectionProps) {
    const [facilities, setFacilities] = useState<AccreditedFacility[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.facilityType || !formData.facilityName || !formData.governorate || !formData.accreditationStatus || !formData.month) { alert('يرجى ملء جميع الحقول'); return; }
    ...
    const handleEdit = (item: AccreditedFacility) => { setFormData({ facilityType: item.facilityType, facilityName: item.facilityName, governorate: item.governorate, accreditationStatus: item.accreditationStatus, month: item.month }); setEditingId(item.id || null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
    ...
    const exportToExcel = () => {
        ...
        const data = filteredData.map((item, i) => { const [year, month] = item.month.split('-'); return { '#': i + 1, 'نوع المنشأة': item.facilityType, 'اسم المنشأة': item.facilityName, 'المحافظة': item.governorate, 'حالة الاعتماد': item.accreditationStatus, 'الشهر': `${monthNames[parseInt(month) - 1]} ${year}` }; });
        ...
        XLSX.writeFile(workbook, `المنشآت_المعتمدة${filterMonthText}.xlsx`);
    };
    ...
    const exportToWord = async () => {
        ...
        const table = new Table({ rows: [new TableRow({ children: [new TableCell({ children: [new Paragraph({ text: '#', alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'نوع المنشأة', alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'اسم المنشأة', alignment: AlignmentType.CENTER })], width: { size: 25, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'المحافظة', alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'حالة الاعتماد', alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } }), new TableCell({ children: [new Paragraph({ text: 'الشهر', alignment: AlignmentType.CENTER })], width: { size: 15, type: WidthType.PERCENTAGE } })] }), ...tableRows], width: { size: 100, type: WidthType.PERCENTAGE } });
        const doc = new Document({ sections: [{ properties: {}, children: [new Paragraph({ text: 'المنشآت المعتمدة خلال الشهر', alignment: AlignmentType.CENTER, spacing: { after: 200 } }), table] }] });
        const blob = await Packer.toBlob(doc);
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `المنشآت_المعتمدة${filterMonthText}.docx`;
        link.click();
    };
    ...
    return (
        <div className="card" style={{ marginTop: '30px' }}>
            <SectionHeader title="المنشآت المعتمدة خلال الشهر" icon="✅" count={filteredData.length} isExpanded={isExpanded} onToggle={() => setIsExpanded(!isExpanded)} />
            {isExpanded && (
                <>
                    {userCanEdit && (
