...
import {
    MedicalProfessionalByGovernorate,
    saveMedicalProfessionalByGovernorate,
    getMedicalProfessionalsByGovernorate,
    updateMedicalProfessionalByGovernorate,
    deleteMedicalProfessionalByGovernorate
} from '@/lib/firestore';

interface MedicalProfessionalsByGovernorateSectionProps {
    currentUser: any;
...
export default function MedicalProfessionalsByGovernorateSection({ currentUser, canEdit }: MedicalProfessionalsByGovernorateSectionProps) {
    const [professionals, setProfessionals] = useState<MedicalProfessionalByGovernorate[]>([]);
    const [editingId, setEditingId] = useState<string | null>(null);
    const [filterMonth, setFilterMonth] = useState('');
    const [isExpanded, setIsExpanded] = useState(false);
    const [formData, setFormData] = useState({
        governorate: '', doctors: '0', dentists: '0', pharmacists: '0', physiotherapy: '0',
    ...
    const loadData = async () => {
        const data = await getMedicalProfessionalsByGovernorate();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.governorate || !formData.month) { alert('يرجى ملء الحقول المطلوبة'); return; }
        ...
        try {
            if (editingId) {
                const success = await updateMedicalProfessionalByGovernorate(editingId, { ...profData, updatedBy: currentUser.email });
                if (success) { await loadData(); resetForm(); alert('تم التحديث'); }
            } else {
                const id = await saveMedicalProfessionalByGovernorate(profData);
    ...
    const handleEdit = (item: MedicalProfessionalByGovernorate) => {
        setFormData({
            governorate: item.governorate, doctors: item.doctors.toString(), dentists: item.dentists.toString(),
    ...
    const handleDelete = async (id: string) => {
        ...
        if (confirm('هل أنت متأكد؟')) {
            const success = await deleteMedicalProfessionalByGovernorate(id);
    ...
    const exportToExcel = () => {
        ...
        const data = filteredData.map((item, i) => {
            ...
            return {
                '#': i + 1, 'المحافظة': item.governorate, 'أطباء بشريين': item.doctors, 'أطباء أسنان': item.dentists,
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'المهنيين حسب المحافظة');
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        XLSX.writeFile(workbook, `المهنيين_الطبيين_المحافظة${filterMonthText}.xlsx`);
    };
    ...
    const exportToWord = async () => {
        ...
        const doc = new Document({ sections: [{ properties: {}, children: [new Paragraph({ text: 'المهنيين الطبيين حسب المحافظة', alignment: AlignmentType.CENTER, spacing: { after: 200 } }), table] }] });
        const blob = await Packer.toBlob(doc);
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        const filterMonthText = filterMonth ? `_${filterMonth.replace('-', '_')}` : '';
        link.download = `المهنيين_الطبيين_المحافظة${filterMonthText}.docx`;
        link.click();
    };
    ...
    return (
        <div className="card" style={{ marginTop: '30px' }}>
            <SectionHeader title="أعضاء المهن الطبية حسب المحافظة" icon="🗺️" count={filteredData.length} isExpanded={isExpanded} onToggle={() => setIsExpanded(!isExpanded)} />
            {isExpanded && (
                <>
                    {userCanEdit && (
                        <form onSubmit={handleSubmit} style={{ marginBottom: '30px', padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
                            <h3 style={{ marginTop: 0, marginBottom: '20px', color: 'var(--secondary-color)' }}>{editingId ? 'تعديل' : 'إض افة'}</h3>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', borderRadius: '8px', overflow: 'hidden', fontSize: '0.9rem' }}>
                            <thead>
                                <tr style={{ backgroundColor: '#0D6A79', color: 'white' }}>
                                    ...
                                    <th style={{ padding: '10px', textAlign: 'center', fontWeight: 'bold' }}>المحافظة</th>
                                ...
                                {filteredData.length === 0 ? (
                                    ...
                                    filteredData.map((item, index) => (
                                        <tr key={item.id} style={{ borderBottom: '1px solid #eee', backgroundColor: index % 2 === 0 ? 'white' : '#f9f9f9' }}>
                                            ...
                                            <td style={{ padding: '10px', textAlign: 'center', fontWeight: '500' }}>{item.governorate}</td>
