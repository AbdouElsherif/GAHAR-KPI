...
export default function TrainingDashboard({ submissions }: TrainingDashboardProps) {
    ...
    const aggregateData = (data: Array<Record<string, any>>, type: 'monthly' | 'quarterly' | 'halfYearly' | 'yearly') => {
        const aggregated: Record<string, { trainingPrograms: number; trainees: number; count: number }> = {};
        ...
        data.forEach(sub => {
            ...
            if (!aggregated[periodKey]) {
                aggregated[periodKey] = { trainingPrograms: 0, trainees: 0, count: 0 };
            }

            aggregated[periodKey].trainingPrograms += parseFloat(sub.trainingPrograms) || 0;
            aggregated[periodKey].trainees += parseFloat(sub.trainees) || 0;
    ...
    const preparePieData = (metric: 'trainingPrograms' | 'trainees') => {
        if (comparisonType === 'yearly' || comparisonType === 'monthly') {
            ...
            const previousVal = metric === 'trainingPrograms' ? previousTotalPrograms : previousTotalTrainees;
            return [
    ...
    const programsPieData = preparePieData('trainingPrograms');
    const traineesPieData = preparePieData('trainees');
    ...
    function prepareChartData() {
        ...
        return sortedPeriods.map(period => {
            ...
            return {
                ...
                [`برامج ${targetYear}`]: currentAggregated[period]?.trainingPrograms || 0,
                [`برامج ${targetYear - 1}`]: previousAggregated[previousPeriodKey]?.trainingPrograms || 0,
                [`متدربين ${targetYear}`]: currentAggregated[period]?.trainees || 0,
                [`متدربين ${targetYear - 1}`]: previousAggregated[previousPeriodKey]?.trainees || 0,
            };
        });
    }
        ...
        <div>
            <div style={{ marginBottom: '30px' }}>
                ...
                <p style={{ margin: 0, color: '#666', fontSize: '1rem' }}>
                    الإدارة العامة للتدريب للغير - تحليلات ومقارنات
                </p>
            </div>
            ...
            <div style={{
                ...
                <KPICard
                    title="إجمالي البرامج التدريبية"
                    icon="📚"
                ...
                <KPICard
                    ...
                    icon="👥"
                    currentValue={currentTotalTrainees}
                    previousValue={previousTotalTrainees}
                    changePercentage={traineesChange}
                    currentYear={targetYear}
                    previousYear={targetYear - 1}
